...
export function CreateProductCarouselImage({ files }: Props) {
    const [loading, setLoading] = useState(false)
    const [clickedId, setClickedId] = useState("")
    // const deleteFile = async (id: string, key: string) => {
    //     setClickedId(id)
    //     setLoading(true)
    //     await fetch(`/api/uploadthing?id=${id}&key=${key}`, { method: 'DELETE' }).finally(() => {
    //         setLoading(false)
    //     })
    // }
            ...
            <CarouselContent>
                {files.map((file, index) => (
                    <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/3 relative">
                        <Button disabled={file.id === clickedId} 
                        // onClick={() => deleteFile(file.id, file.key)} 
                        className="absolute right-0 top-0" variant="outline" size="icon">
                            <X size={10} color="#000" />
                        </Button>
